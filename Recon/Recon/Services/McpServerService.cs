    ...
    private async Task<McpResponse> HandleMcpRequestAsync(McpRequest request)
    {
        ...
        try
        {
            switch (request.Method)
            {
                ...
                case "tools/call":
                    {
                        var toolName = request.Params?.ValueKind == JsonValueKind.Object
                            ? request.Params.Value.GetProperty("name").GetString()
                            : null;
                        var arguments = request.Params?.ValueKind == JsonValueKind.Object
                            ? request.Params.Value.GetProperty("arguments")
                            : (JsonElement?)null;

                        if (toolName == "query_loans")
                        {
                            var queryRequest = arguments.HasValue
                                ? JsonSerializer.Deserialize<QueryRequest>(arguments.Value.GetRawText(), _jsonOptions)
                                : new QueryRequest();
